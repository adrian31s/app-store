...
# manual run
*** not needed ***
docker pull adrian31s/app-store-ui:1.0.10
docker pull adrian31s/app-store:1.0.10
...
docker run -d -e DB_URL=jdbc:postgresql://app_postgres/appstore -e DB_PASSWORD=store-password -e DB_USER=store-user -e HIBERNATE_GENERATION=update -p 30001:8080 --network app-store-network adrian31s/app-store:1.0.10

docker run -d -p 30002:80 --network app-store-network adrian31s/app-store-ui:1.0.10
